# 📊 **Automated Attendance Sheet & Salary Management System**

A fully connected Excel-based attendance ecosystem powered by a smart **Master Attendance Sheet** and an interactive **PowerBI Dashboard**.
Designed to reduce manual errors, automate calculations, and provide clear salary insights — all in one place. ⚡✔️

---

## ⭐ **Key Highlights**

### 🔗 **Integrated Workflow**

* Connects multiple **ALS, BLS, and Event Daily Reports** into one centralized system.
* Extracts **Call Count, DRB Reports, Cancel/Return Calls**, and other metrics.

### 📘 **Master Attendance Sheet**

* All department data flows into a single consolidated source.
* Handles daily data like:

  * Present / Absent
  * Half-Days
  * Out-of-Call amounts
  * Event & DRB values

### 💰 **Automated Salary Calculation**

* Calculates:

  * **Basic Salary**
  * **Deductions**
  * **Half-Day Impact**
  * **Call Amount Earnings**
  * **Final Salary**
* Removes manual calculations entirely.

### 📈 **PowerBI Dashboard**

A clean and interactive dashboard showing:

* 🧑‍💼 Employee-wise salary summary
* 📅 Month-wise performance
* 💸 Salary vs Deduction vs Final Salary
* 📞 Call-based earnings
* 🔔 Final salary output clearly highlighted

---

## 📸 **Dashboard Preview**

> *<img width="1219" height="688" alt="image" src="https://github.com/user-attachments/assets/f3a33b9a-8c8c-43c5-b84a-3af22d968ce6" />
*

```
!<img width="1197" height="618" alt="image" src="https://github.com/user-attachments/assets/01330904-31b9-4ab6-bd8a-aaaee6b06795" />
```

---

## 🚀 **What This System Solves**

* ❌ No more manual attendance calculation
* ❌ No separate sheets for salary, calls, or deductions
* ✔️ One master sheet for everything
* ✔️ Automated salary output
* ✔️ Visual insights through PowerBI

---

## 🧰 **Tech Used**

* 🟦 Microsoft Excel (Data Source)
* 🟨 PowerBI (Visualization & Reporting)

---

## 💡 **Simple • Automated • Insightful**

This system transforms attendance data into clear, actionable insights—saving time and improving accuracy. 🌟
